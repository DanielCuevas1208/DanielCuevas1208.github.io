...
for (const skill of source) {
  ...
  released += 1;
  const groups = serverField(skill, 'global', 'condition_groups');
  const alternatives = (Array.isArray(groups) ? groups : [])
  ...
  const activation = Number(serverField(skill, 'global', 'activation') || 0);
  out[String(id)] = {
    alternatives,
    rarity: Number(skill.rarity || 1),
    wisdomCheck: activation,
  };
}

fs.mkdirSync(path.dirname(outPath), { recursive: true });
fs.writeFileSync(outPath, `${JSON.stringify(out)}\n`, 'utf8');
process.stdout.write(`Global skill_data: ${Object.keys(out).length} evaluable / ${released} released; ${skipped} unreleased; ${noGroups} without usable groups.\n`);
